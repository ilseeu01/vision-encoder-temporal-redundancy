...
L.append("또한 정규화 인코딩 변화를 픽셀 변화로 나눈 **픽셀 대비 증폭률(%)** 을 함께 싣는다. "
         "픽셀(휘도 범위 대비 비율)도 인코딩(토큰 크기 대비 비율)도 무차원이라 단위가 일관되며, "
         "값이 클수록 같은 입력 변화에 인코더가 더 크게 반응(더 민감)한다. 정적 기준 SAM2.1이 "
         "가장 작고(~1,555%) Qwen2.5가 가장 크다(~5,840%).\n")
L.append("| 모델 | 영상 | 토큰 norm (L1 / L2) | 정규화 인코딩Δ (L1 / L2) | 픽셀 대비 증폭률 (L1 / L2, %) |")
L.append("|---|---|---|---|---|")
for r in rows:
    for v in ("fast", "static"):
        d = r.get(v)
        ...
        pix = d["pix"]["l1"]; a1 = en.get("l1"); a2 = en.get("l2")
        amp1 = f"{a1 / pix * 100:,.0f}%" if a1 else "—"
        amp2 = f"{a2 / pix * 100:,.0f}%" if a2 else "—"
        L.append(f"| {r['name']} | {v} | {f(tn.get('l1'))} / {f(tn.get('l2'),2)} | "
                 f"{f(en.get('l1'),4)} / {f(en.get('l2'),4)} | {amp1} / {amp2} |")
